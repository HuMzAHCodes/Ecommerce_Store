...
const Checkout = () => {
  ...
  const [shipping, setShipping] = useState({ firstName:"",lastName:"",email:"",phone:"",address:"",city:"",state:"",zip:"",country:"Pakistan" });
  ...
  const FieldErr = ({ msg }:{msg?:string}) =>
    msg ? <p style={{ fontFamily:typography.fontBody,fontSize:"0.68rem",color:colors.error,marginTop:3 }}>{msg}</p> : null;
    ...
    <div style={{ background: colors.bgPrimary, minHeight:"100vh" }}>
      ...
      <div style={{ background: colors.bgSecondary, borderBottom:`1px solid ${colors.borderLight}`, padding: isMobile ? "1.5rem 1.25rem" : "2rem 1.5rem" }}>
        <div style={{ maxWidth:1280, margin:"0 auto" }}>
          <h1 style={{ fontFamily:typography.fontDisplay, color:colors.textPrimary, fontStyle:"italic", marginBottom:"1.25rem", fontSize: isMobile ? typography["2xl"] : undefined }}>Checkout</h1>
          <div style={{ display:"flex", alignItems:"center", gap: isMobile ? "0.375rem" : "0.5rem", flexWrap:"wrap" }}>
            {STEPS.map(({ id, label }, i) => {
              const done = i < currentIndex; const active = id===step;
              return (
                <div key={id} style={{ display:"flex", alignItems:"center", gap: isMobile ? "0.25rem" : "0.5rem" }}>
                  <div style={{ display:"flex", alignItems:"center", gap: isMobile ? 4 : 6 }}>
                    <div style={{ width:24, height:24, borderRadius: radius?.full, background: done ? colors.success : active ? colors.accentPrimary : colors.borderMedium, display:"flex", alignItems:"center", justifyContent:"center", transition:`background ${transitions?.normal}` }}>
                      {done ? <Check size={12} color="#fff"/> : <span style={{ fontFamily:typography.fontBody,fontSize:"0.65rem",fontWeight:typography.weightBold,color:"#fff" }}>{i+1}</span>}
                    </div>
                    <span style={{ fontFamily:typography.fontBody, fontSize: isMobile ? typography.xs : typography.sm, fontWeight: active ? typography.weightMedium : typography.weightRegular, color: active ? colors.textPrimary : colors.textMuted }}>
                      {label}
                    </span>
                  </div>
      ...
      <div style={{
        ...
        <AnimatePresence mode="wait">
          {step==="shipping" && (
            <motion.div key="shipping" initial={{ opacity:0, x:20 }} animate={{ opacity:1, x:0 }} exit={{ opacity:0, x:-20 }} transition={{ duration:0.28 }}
              ...
              <div style={{ display:"grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap:"0.875rem" }}>
                {([["firstName","First Name",""],["lastName","Last Name",""],["email","Email","full"],["phone","Phone","full"],["address","Address","full"],["city","City",""],["state","State",""],["zip","ZIP Code",""]] as [keyof typeof shipping,string,string][]).map(([field,label,span]) => (
                  <div key={field} style={{ gridColumn: span==="full"&&!isMobile ? "1/-1" : undefined }}>
                    <Label>{label}</Label>
          ...
          {step==="payment" && (
            <motion.div key="payment" initial={{ opacity:0, x:20 }} animate={{ opacity:1, x:0 }} exit={{ opacity:0, x:-20 }} transition={{ duration:0.28 }}
              style={{ background:colors.bgCard,borderRadius:radius?.xl,padding: isMobile?"1.25rem":"2rem",border:`1px solid ${colors.borderLight}`,boxShadow:shadows?.sm }}>
          ...
          {step==="review" && (
            <motion.div key="review" initial={{ opacity:0, x:20 }} animate={{ opacity:1, x:0 }} exit={{ opacity:0, x:-20 }} transition={{ duration:0.28 }}
              style={{ background:colors.bgCard,borderRadius:radius?.xl,padding: isMobile?"1.25rem":"2rem",border:`1px solid ${colors.borderLight}`,boxShadow:shadows?.sm }}>
              ...
              {items.map(({ product, quantity }) => (
                <div key={product.id} style={{ display:"flex",alignItems:"center",gap:"0.875rem",padding:"0.75rem 0",borderBottom:`1px solid ${colors.borderLight}` }}>
                  <div style={{ width:48,height:48,borderRadius:radius?.md,background:product.image,display:"flex",alignItems:"center",justifyContent:"center",fontSize:"1.25rem",flexShrink:0 }}>✨</div>
                  <div style={{ flex:1 }}>
        ...
        <motion.div initial={{ opacity:0, y:16 }} animate={{ opacity:1, y:0 }} transition={{ duration:0.4, delay:0.15 }}
          style={{ background:colors.bgCard,borderRadius:radius?.xl,padding:"1.5rem",border:`1px solid ${colors.borderLight}`,boxShadow:shadows?.sm, ...(!isMobile ? { position:"sticky", top:84 } : {}) }}>
          <h3 style={{ fontFamily:typography.fontDisplay,fontSize:typography.xl,color:colors.textPrimary,marginBottom:"1.1rem" }}>Order Summary</h3>
          {items.map(({ product, quantity }) => (
            <div key={product.id} style={{ display:"flex",justifyContent:"space-between",marginBottom:"0.5rem",fontFamily:typography.fontBody,fontSize:typography.sm }}>
              <span style={{ color:colors.textSecondary,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",maxWidth:"60%" }}>{product.name} × {quantity}</span>
              <span style={{ color:colors.textPrimary,fontWeight:typography.weightMedium,flexShrink:0 }}>${((product.salePrice??product.price)*quantity).toFixed(2)}</span>
            </div>
          ))}
