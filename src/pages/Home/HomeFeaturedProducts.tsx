...
const HomeFeaturedProducts = () => {
  ...
  return (
    // CHANGED: bgPrimary (#352A22) instead of bgSecondary.
    // After the bright bgTertiary Categories section, dropping back to the
    // darkest tone creates contrast and makes the product cards pop.
    <section style={{ background: colors.bgPrimary, padding: "5rem 0" }}>
      <div style={{ maxWidth: 1280, margin: "0 auto", padding: "0 1.5rem" }}>
            ...
            <ScrollReveal key={product.id} y={44}>
              <Link to={`/shop/${product.slug}`} style={{ textDecoration: "none", display: "block" }}>
                <motion.div
                  whileHover={{ y: -4 }}
                  transition={{ duration: 0.22 }}
                  style={{ background: colors.bgCard, borderRadius: radius?.xl, overflow: "hidden", boxShadow: shadows?.sm, // CHANGED: borderMedium (18% opacity) instead of borderLight (10%) — cards pop more on dark bgPrimary
                  border: `1px solid ${colors.borderMedium}`, cursor: "pointer" }}
                >
                  {/* Product image area */}
                  <div style={{ height: 220, background: product.bg, display: "flex", alignItems: "center", justifyContent: "center", position: "relative" }}>
                    {product.image ? (
